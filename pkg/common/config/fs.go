...
func parseFSConfig(cfg *Config) error {
	...
	parsers := []configParser{parseFuseConfig, parseBackgroundFetchConfig, parseRetryableHTTPClientConfig, parseBlobConfig, parseContentStoreConfig, parseExternalArtifactStoreConfig}
...
func parseExternalArtifactStoreConfig(cfg *Config) error {
	if cfg.ExternalArtifactStoreConfig.TimeoutSec == 0 {
		cfg.ExternalArtifactStoreConfig.TimeoutSec = 2
	}
	if cfg.ExternalArtifactStoreConfig.Enable && cfg.ExternalArtifactStoreConfig.Endpoint == "" {
		return errors.New("external_artifact_store.endpoint must be set when external_artifact_store.enable is true")
	}
	return nil
}
