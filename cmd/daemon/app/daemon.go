...
import (
	...
	"github.com/cloudpilot-ai/hermes/pkg/daemon/fs"
	hermesgrpc "github.com/cloudpilot-ai/hermes/pkg/daemon/grpc"
	"github.com/cloudpilot-ai/hermes/pkg/daemon/grpc/keychain/cri/v1"
	crialpha "github.com/cloudpilot-ai/hermes/pkg/daemon/grpc/keychain/cri/v1alpha"
	"github.com/cloudpilot-ai/hermes/pkg/daemon/metadata"

	"github.com/cloudpilot-ai/hermes/pkg/common/soci/ztoc"
	"github.com/cloudpilot-ai/hermes/pkg/common/version"
	"github.com/cloudpilot-ai/hermes/pkg/daemon/grpc/keychain/dockerconfig"
	"github.com/cloudpilot-ai/hermes/pkg/daemon/grpc/keychain/kubeconfig"
	"github.com/cloudpilot-ai/hermes/pkg/daemon/grpc/resolver"
...
func NewDaemonCommand(_ context.Context) *cli.Command {
	...
	return &cli.Command{
		...
		Action: func(ctx context.Context, cmd *cli.Command) error {
			...
			logrus.SetLevel(opts.ParsedLogLevel)
			...
			if !cfg.SkipCheckSnapshotterSupported {
				if err := hermesgrpc.Supported(rootDir); err != nil {
			...
			rs, err := hermesgrpc.NewSociSnapshotterService(ctx, rootDir, &cfg.ServiceConfig,
				hermesgrpc.WithCredsFuncs(credsFuncs...), hermesgrpc.WithFilesystemOptions(fsOpts...))
			if err != nil {
				log.G(ctx).WithError(err).Fatalf("failed to configure snapshotter")
				return err
			}

			cleanup, err := serve(ctx, rpc, opts.Address, rs, *cfg)
			if err != nil {
...
func serve(ctx context.Context, rpc *grpc.Server, addr string, rs snapshots.Snapshotter, cfg config.Config) (bool, error) {
	...
	log.G(ctx).WithField("address", addr).Info("hermes-daemon successfully started")
